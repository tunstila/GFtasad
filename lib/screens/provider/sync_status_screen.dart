...
class _SyncStatusScreenState extends State<SyncStatusScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final pendingPrevention = prevention.records.where((r) => r.syncStatus != SyncStatus.synced).toList();
    ...
    final totalPending = pendingRecords.length + pendingPrevention.length + pendingMovements.length + pendingDeliveries.length;
                ...
                : () async {
                    ...
                    try {
                      await Future.wait([
                        records.syncNow(),
                        prevention.syncNow(),
                        inventory.markAllPendingAsSynced(),
                        deliveries.markAllPendingAsSynced(providerId: userId),
                      ]);
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            ...pendingRecords.map((r) => _PendingRow(type: 'Test record', timestamp: r.createdAt, status: r.syncStatus)),
            ...pendingPrevention.map((r) => _PendingRow(type: 'Prevention messaging', timestamp: r.createdAt, status: r.syncStatus)),
